...
const footerLinks = [
  ...
  {
    title: 'Resources',
    links: ['University Rankings', 'Application Guides', 'Scholarship Information', 'Blogs']
  },
  {
    title: 'Legal & Support',
...
const Footer: React.FC = () => {
  return (
    <footer className="mt-[300px] w-full py-[80px] bg-[#001124]">
      <div className="flex flex-col items-center w-[80%] mx-auto gap-y-[50px] max-[1000px]:gap-y-[50px]">
        <div className="flex flex-col gap-y-[20px] w-full max-[900px]:w-[90%]">
        ...
        <div className="w-full grid grid-cols-4 gap-[30px] max-[900px]:grid-cols-2 max-[400px]:flex-col">
          {footerLinks.map(({ title, links }) => (
            <div key={title} className="flex flex-col gap-y-[15px]">
              ...
              {links.map(link => (
                <div key={link} className="text-xs">{link}</div>
              ))}
            </div>
          ))}
        </div>
