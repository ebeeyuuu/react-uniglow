...
interface SidebarItemProps {
  item: NavigationItem;
  parentPath: string[];
  onNavigate: (path: string[], component: React.ReactNode) => void;
  isCollapsed: boolean;
}
...
const SidebarItem: React.FC<SidebarItemProps> = ({
  ...
  const handleClick = (e: React.MouseEvent) => {
    ...
    if (hasChildren && isCollapsed) {
      ...
      setIsOpen(!isOpen);
      onNavigate(currentPath, item.component);
    } else {
      onNavigate(currentPath, item.component);
    }
  };
          ...
          >
            {item.children!.map((child) => (
              <DropdownMenuItem
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  onNavigate([...currentPath, child.name], child.component);
                  setIsOpen(false);
                }}
                selected={false}
              >
                {child.icon && (
                  <span className="w-4 h-4 text-zinc-400 mr-2">
                    {child.icon}
                  </span>
                )}
    ...
    <li>
      {item.href ? (
        <Link
          href={item.href}
          className="block"
          onClick={() => onNavigate(currentPath, item.component)}
        >
          {content}
        </Link>
      ) : (
