...
export function Calendar({ containerClassName, size = "md" }: CalendarProps) {
  ...
  return (
    <div
      className={cn(
        "bg-zinc-800 text-white p-4 rounded-lg",
        container,
        containerClassName,
      )}
    >
      <div className="flex justify-between items-center mb-4">
        <button
          ...
          className={cn("text-gray-400 hover:text-white", navButton)}
        >
          &lt;
        </button>
        <h2 className={cn("font-bold", monthYear)}>
      ...
      <div className="grid grid-cols-7 gap-1 mb-2">
        {DAYS.map((day) => (
          <div
            key={day}
            className={cn("text-center font-medium text-gray-400", dayName)}
          >
            {day}
          </div>
        ))}
